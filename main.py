...
Excel workbook with both per-device sheets and a global SUMMARY sheet.

Key features:
- Uses TextFSM parsing of 'show interfaces' for detailed port data
- Normalizes interface names for consistent mapping across commands
- Enriches with PoE details, LLDP/CDP neighbors, and error counters
- Flags "stale" access ports based on configurable inactivity criteria
